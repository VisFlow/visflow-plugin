...
function resolveWorkspaceMemberForFile(workspaceRoot, cwd, file) {
  const loaded = loadWorkspace(workspaceRoot);
  if (!loaded.ok) return null;
  const lexical = isAbsolute2(file) ? resolve2(file) : resolve2(cwd, normalize2(file));
...
function resolveRepositoryFile(repoRoot, cwd, file) {
...
function handleEdit(payloadRaw, env, now) {
  ...
  if (context.kind === "workspace") {
    const owner = resolveWorkspaceMemberForFile(context.workspaceRoot, p.cwd, file);
    if (!owner) return;
  ...
  const resolved = resolveRepositoryFile(repoRoot, p.cwd, file);
  if (!resolved) return;
  const eventFile = repoRoot === p.cwd ? file : resolved.relativePath;
